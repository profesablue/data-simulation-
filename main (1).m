...
table_current = array2table([string(lTime'), current_abs_val], 'VariableNames', {'Time', 'Calculated Current'});
...
yyaxis left
plot(lTime, current_abs_val, '-r', 'LineWidth', 1.5);
hold on 
...
if exist('Res', 'var')
    if length(lTime) == length(Res.T_avg)
        plot(lTime', Res.T_avg, '-k', 'LineWidth', 1.5);
    else
